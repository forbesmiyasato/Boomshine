...
public class MultiballExplode {
    ...
    public ArrayList<ExplodingBoundedMovingCircle> createMulti(ExplodingType eType,
                                                               Context context, Display display,
                                                               int drawable, int topCoord,
                                                               int leftCoord, int speed,
        ...
        int randomXCoord = cRandom.nextInt(cDisplaySize.width() -
                BoomshineView.DEFAULT_BALL_RADIUS * 2) + BoomshineView.DEFAULT_BALL_RADIUS;
        int randomYCoord = cRandom.nextInt(cDisplaySize.height() -
                BoomshineView.DEFAULT_BALL_RADIUS * 2) + BoomshineView.DEFAULT_BALL_RADIUS;

        cRetList.add (new ExplodingBoundedMovingCircle(eType, context, display, drawable,
                topCoord, leftCoord, speed, topBound, bottomBound, leftBound, rightBound,
                radius));

        cRetList.add (new ExplodingBoundedMovingCircle(eType, context, display, drawable,
                randomYCoord, randomXCoord, speed, topBound, bottomBound, leftBound, rightBound,
                radius));
